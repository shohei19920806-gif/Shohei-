...
楽天アフィリエイトと並行してGoogleアドセンス（クリック報酬型広告）にも
申請できます。ただしAI生成記事だけでは審査に通りにくいため、次の対応をしています。

- `generate` された記事は、既存のE-E-A-T対策（体験談の目印）・文章品質ルールにより、
  Google側が問題視する「量産感のある無価値なAI文章」になりにくい設計にしている
- 審査に必須とされることが多い固定ページ（プライバシーポリシー・お問い合わせ・
  運営者プロフィール）を自動生成する `setup-pages` コマンドを用意
- 申請前の準備状況（公開記事数・固定ページの有無）を確認する `adsense-checklist` コマンドを用意

```bash
# 固定ページを作成（デフォルトは下書き。中身を必ず自分の言葉で確認してから--publish）
python -m affiliate_blog_tool.cli setup-pages \
  --site-name "育児と釣りブログ" --operator-name "しょうへい" \
...
**注意**: `setup-pages` が生成するプライバシーポリシー等の文章は叩き台です。
実態と異なる記載がないか、公開前に必ず自分で確認・編集してください。
また、YMYL（お金・健康に関わる）ジャンルはアドセンス審査が特に厳しくなる傾向があるため、
育児ジャンルで医療的な断定表現をしないという既存のルールは審査対策としても有効です。

## E-E-A-T対策：あなた自身の体験談を追記する

Googleは「経験（Experience）」を重視する評価基準（E-E-A-T）を採用しているため、
AIが書いた一般論だけの記事より、筆者本人の一次体験が書かれた記事の方が評価されやすい
傾向にあります。

`generate` で作られる記事には、自動で `<!-- EXPERIENCE: 何を書くべきかの一言ヒント -->`
という目印が1箇所挿入されます。**公開前に、このヒントに沿ってあなた自身の実体験
（写真・エピソード・感想）を書き足す**ことを推奨します。

`publish-wp --publish` 実行時に、この目印が本文に残っていると警告が表示されます
（公開を止めるものではなく、あくまで気づくためのリマインドです）。
